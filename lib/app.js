...
app.get('/animals', async(req, res) => {
  try {
    ...
    SELECT animals.*, temperaments.temperament
    FROM animals
    JOIN temperaments 
    ON animals.temperament_id=temperaments.id
    `);
    res.json(data.rows);
  } catch(e) {
...
app.put('/animals/:id', async(req, res) => {
  ...
  try {
    ...
    const data = await client.query(`
      UPDATE animals
      SET species = $1, temperament_id = $2
      WHERE id=$3
      returning *;
    `,
